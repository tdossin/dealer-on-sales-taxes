<template>
	<section class="row d-flex gap-5 justify-content-center">
		<div class="col-md-9">
			<h2>Solution</h2>
		</div>
	</section>
	<section id="cart" class="row d-flex gap-5 justify-content-center">
		<div class="col-sm-4">
			<h4>Click to select your items for purchase:</h4>
			<ul class="list-group">
				<li
					v-for="item in taxedItems"
					:key="item.id"
			...
			<div class="row">
				<div class="col-md-12 notes">
					<p>
						<span class="tag taxable">t</span>: Taxable (+10% sales tax applies)
					</p>
					<p>
						<span class="tag imported">i</span>: Imported item (+5% import tax
						applies)
					</p>
				</div>
			</div>
		...
		<div class="col-4">
			...
			<div class="row">
				<p v-for="item in countedCart" :key="item.id">
					{{ item.name }}:
					<span v-if="item.count <= 1">{{
						toCurrency(item.price + item.importTaxes + item.salesTaxes)
					}}</span>
					<span v-else>
						{{
							toCurrency(
								(item.price + item.importTaxes + item.salesTaxes) * item.count
							)
						}}
						({{ item.count }} @
						{{ toCurrency(item.price + item.importTaxes + item.salesTaxes) }})
			...
			<div class="row" v-if="cart.length">
				<div class="col-6">
					<p>Sales Taxes: {{ toCurrency(totalSalesTaxes) }}</p>
					<p>Total: {{ toCurrency(totalSale) }}</p>
					<button @click="emptyCart" type="button" class="btn btn-primary">
						Empty Cart
					</button>
				</div>
			</div>
